...
output=/stor1/hwang07/paper_collection/collect_all_configs_warp_partition2_tsp_figure15.txt
...
do
cd $benchmark
grep -o "$statistics[ ]*[-eE\+0-9\.]*" output_* | tail -1 | sed -e "s/$statistics[ ]*(-\b)?//g" | xargs printf "0%s " >> $output
cd ..
done
